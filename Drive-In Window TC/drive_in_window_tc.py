# This interpreter modified by BoundedBeans for Drive-In Window TC
# and Python 3.
#----------------------------------------------------------------------
...
while line_number < len(commands):
    ...
    if declarationState == 'start':
            ...
            if split_line[0] or (not line[-5:] == ' has.') or (not len(split_line)==3):
                ...
                if success:
                    if not modifier:
                        ...
                        people[person1] += people[person2]
                    #Fix for a weird bug in the original interpreter where not having "also" or "not" would result in an error.
                    elif modifier == 'like what':
                        people[person1] = people[person2]
                    else:
                        success = False
        ...
        #Person N would like to get takeout.
        elif 'Person ' in line and ' would like to get takeout.' in line:
            person = line.replace('Person ','').replace(' would like to get takeout.','').strip()
            for digit in person:
                if not digit in digits:
                    success = False
                    errorMessage = 'Person must be a positive whole number.'
            ...
            if success:
                person = int(person)
                if person not in people_stacks:
                    people_stacks[person] = []
                try:
                    people_stacks[person].append(people[person])
                except KeyError:
                    errorMessage = 'Can\'t put a nonexistant value into a stack.'
                    success = False
        #Person N would like to bring in food.
        elif 'Person ' in line and ' would like to bring in food.' in line:
            person = line.replace('Person ','').replace(' would like to bring in food.','').strip()
            for digit in person:
                if not digit in digits:
                    success = False
                    errorMessage = 'Person must be a positive whole number.'
            ...
            if success:
                person = int(person)
                if person not in people_stacks or len(people_stacks[person]) == 0:
                    errorMessage = 'Person' + str(person) + '\'s stack was empty.'
                    success = False
            if success:
                people[person] = people_stacks[person][-1]
                people_stacks[person].pop()
        ...
        #Person N will pay for his order!
            ...
            if split_line[0]:
                ...
                #Print person value:
                if success:
                    sys.stdout.write(str(chr(people[person])))
        ...
        #OK, what should Person N get?
            ...
            if success:
                ...
                if person == 0:
                    ...
                    errorMessage = 'Person must be greater than 0'
            if success:
                #Terrible solution for char input but sys.stdin.read(1) wasn't working for some reason
                if input_queue == '':
                    input_queue = input() + '\n'
                else:
                    people[person] = ord(input_queue[0])
        ...
        #OK, how much money should Person N have?
            ...
            if success:
                try:
                    ...
                    print('Expected integer input, using 0.')
                    people[person] = 0
        ...
        #Just wait while we decide...
            ...
            #raw_input()
            print('The "Just wait while we decide..." command is not implemented in this modified interpreter so that running it on windows does not require MSVC++.')
        #Person N is leaving a review: Comment.
        elif 'Person ' in line and ' is leaving a review:' in line:
            pass
        #OK, that will be $X.XX. Thanks for coming!
